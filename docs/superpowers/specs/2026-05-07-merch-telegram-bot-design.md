...
- `sizes` — CSV. Пусто или `-` = без размера, шаг выбора пропускается.
- `active=FALSE` скрывает товар, не удаляя
- `id` не должен содержать `:` (используется в callback_data)
...
### `Orders`
| order_id | chat_id | name | phone | address | comment | items_json | total | status | receipt_file_id | receipt_kind | reject_reason | created_at | updated_at |

**Статусы:** `awaiting_payment` → `awaiting_receipt` → `pending_review` → `paid` / `rejected` → `shipped` → `done`.

- `items_json` — массив `[{product_id, title, size, qty, price, subtotal}]` (snapshot цены на момент заказа)
- `receipt_file_id` — Telegram `file_id` чека (сам файл хранится у Telegram, не у нас)
- `receipt_kind` — `'photo'` или `'document'` (PDF тоже принимается)
- `reject_reason` — причина отклонения оплаты, если статус `rejected` (комментарий клиента не затирается)
- `order_id` генерируется под `LockService` для защиты от race condition
...
idle ──/start──▶ idle (главное меню: Каталог / Корзина / Мои заказы / Помощь)
  ...
  └──[confirming_order: «Подтвердить»]──▶ awaiting_payment (показ MBank/QR)
                                                │
                                                ├──[«Я оплатил»]──▶ awaiting_receipt (ждём фото или PDF)
                                                │                          │
                                                │                          └──[файл получен]──▶ pending_review ──▶ уведомление админу
                                                │                                                                          │
                                                │                                                  ┌───────────────────────┤
                                                │                                                  ▼ (callback от админа)  ▼
                                                ...
                                                │                                                  │                        │
                                                │                                                  │                клиент видит причину
                                                │                                                  │                + кнопку «Повторить оплату»
                                                │                                                  │                        │
                                                │                                                  │           ┌────────────┴──────┐
                                                │                                                  │           ▼                   │
                                                │                                                  │   awaiting_payment ◀──────────┘
                                                │                                                  │           (можно выслать чек снова)
                                                │                                                  └─клиент уведомлён──┘
                                                │
                                                └──[«Отменить»]──▶ idle
```

- `/cancel` и `/start` сбрасывают FSM в `idle` (корзину не трогает).
- Во время checkout-диалога **FSM-ввод приоритетнее** клика по reply-кнопке меню — случайный «📦 Каталог» при заполнении адреса воспринимается как текст, дилог не прерывается.
- Главное меню — ReplyKeyboard внизу: `📦 Каталог · 🛒 Корзина · 📋 Мои заказы · ℹ️ Помощь`.
- На шаге запроса телефона ReplyKeyboard временно заменяется на кнопку «📱 Поделиться номером», после получения номера главное меню возвращается.
- Корзина показывает позиции одним сообщением + кнопки: `Оформить заказ`, `Очистить корзину`.
- Чек принимается как **фото** (`msg.photo`) или **документ** (`msg.document`, в т.ч. PDF).
...
**Отклонить:** второй ряд кнопок — типовые причины (`Чек не читается` / `Сумма не совпадает` / `Платёж не найден`) или `Своя причина` (бот просит ввести текст). Клиенту: «Оплата не подтверждена: <причина>» + инлайн-кнопка **🔁 Повторить оплату** — возвращает заказ в `awaiting_payment`, можно выслать другой чек.
...
- `cat:<id>` / `cat:back` — выбор категории / возврат к списку категорий
- `prod:<id>` — выбор товара
- `size:<prod_id>:<S>` — выбор размера
- `qty:<n>` / `qty:cancel` — выбор количества / отмена
- `cart:clear` / `cart:checkout`
- `order:confirm` / `order:cancel`
- `pay:done` — клиент нажал «Я оплатил»
- `pay:retry:<order_id>` — повторить оплату после отклонения
- `admin:approve:<order_id>` / `admin:reject:<order_id>` / `admin:reason:<order_id>:<code>`

**Авторизация админа:** проверка по `cb.from.id` (а не `cb.message.chat.id`) — устойчиво к форварду кнопок в группу.
...
- Автосохранение чеков в Google Drive (сейчас живут только на серверах Telegram через `file_id`)
